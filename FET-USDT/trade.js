...
if(localStorage.getItem('fetTradeLogBuy')) {
    trade.tradeLogBuy = JSON.parse(localStorage.getItem('fetTradeLogBuy'));
}

if(localStorage.getItem('fetTradeLogSell')) {
    trade.tradeLogSell = JSON.parse(localStorage.getItem('fetTradeLogSell'));
}
...
const fetPriceDOM = document.querySelector('.coin-price');

setInterval(() => {
    trade.coins.map((coin) => {
        let coinsLastPrice = coin.price

        if(coin.symbol == "FET-USDT") {
            coin.price += (Math.random() * ((coin.price * 0.5) / 100)) - (Math.random() * ((coin.price * 0.49) / 100));
            fetPriceDOM.textContent = `$ ${coin.price.toFixed(4)}`;

            if(coin.price > coinsLastPrice) {
                fetPriceDOM.classList.remove('decreasing');
                fetPriceDOM.classList.add('increasing');
            } else {
                fetPriceDOM.classList.remove('increasing');
                fetPriceDOM.classList.add('decreasing');
            }
...
const currentFetDOM = document.querySelector('.current-coin');

trade.coins.map((coin) => {
    if(coin.symbol == "FET-USDT") {
        currentFetDOM.innerHTML = `${coin.amount.toFixed(2)} FET`;
        fetPriceDOM.innerHTML = `$ ${coin.price.toFixed(4)}`;
    }
})
...
buy = (e) => {
    ...
    if(money >= price) {
        ...
        trade.coins.map((coin) => {
            ...
            localStorage.setItem('fetTradeLogBuy', JSON.stringify(trade.tradeLogBuy));
        }
        });
    } else {
    ...
    trade.coins.map((coin) => {
        if (coin.symbol == "FET-USDT") {
            if(parseInt(amount) <= coin.amount) {
                ...
                localStorage.setItem('fetTradeLogSell', JSON.stringify(trade.tradeLogSell));

                currentFetDOM.textContent = `${coin.amount.toFixed(2)} FET`;
                moneyDOM.textContent = `${money.toFixed(2)} USDT`
            } else {
                alert('Not enough FET');
            }
        }
    });
