...
coins.coins.map((coin) => {
  ...
  coin.isIncreasing
    ? tr.classList.add("increasing")
    : tr.classList.add("decreasing");

  tr.innerHTML = `
        <td class="symbol-td coin-td">${coin.symbol}</td>
        <td class="name-td coin-td"><img src="./images/${
          coin.name
        }.png" class="coin-img">${coin.name}</td>
        <td class="price-td coin-td ${coin.name}-price">$ ${coin.price.toFixed(
    9
  )}</td>
        <td class="buy-coin-td coin-td"><a href="./${
          coin.symbol
        }/trade.html" class="buy-coin-btn">Buy / Sell</a></td>
        <td class="add-to-fav-td coin-td"><div class="add-to-fav-btn"><i class="${
          coin.isOnFavorites ? "fas fa-heart active" : "far fa-heart"
        } add-to-fav"></i></div></td>
    `;
...
setInterval(() => {
  coins.coins.forEach((coin) => {
    ...
    if (coin.name == "Bitcoin") {
      coin.price += Math.floor(
        Math.random() * ((coin.price * 0.5) / 100) -
          Math.random() * ((coin.price * 0.5) / 100)
      );
      let differenceNumber = coin.price - coin.lastPrice;
      let differencePercentage = (
        (differenceNumber / coin.price) *
        100
      ).toFixed(2);

      btcPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed()}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      coin.price +=
        Math.random() * ((coin.price * 0.5) / 100) -
        Math.random() * ((coin.price * 0.49) / 100);
      let differenceNumber = coin.price - coin.lastPrice;
      let differencePercentage = (
        (differenceNumber / coin.price) *
        100
      ...
      dogePriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        5
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      let DOM = document.querySelector(`.${coin.symbol}-difference`);
      DOM.style.width = "64px";
      ...
      shibPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        9
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      ripplePriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        4
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      tronPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        5
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      ethereumPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed()}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      let DOM = document.querySelector(`.${coin.symbol}-difference`);
      DOM.style.width = "64px";
      ...
      litePriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        2
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      dotPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        2
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
      ...
      fetchPriceDOM.innerHTML = `<div style="width: 128px; display: inline-flex;">$ ${coin.price.toFixed(
        4
      )}</div> <div style="margin-left: 8px;" class="${
        coin.symbol
      }-difference">${differencePercentage}%</div>`;
  ...
  if (confirm("Are you sure about reset game? (reload page after reset)")) {
    ...
    coins.coins.map((coin) => {
      ...
      tr.innerHTML = `
                ...
                <td class="price-td coin-td ${coin.name}-price">$ ${
        coin.price
      }</td>
                <td class="buy-coin-td coin-td"><a href="./${
                  coin.symbol
...
showNewCoins = (e) => {
  Array.from(allCoins).forEach((coin) => {
    for (let x = 0; x < coins.coins.length; x++) {
      if (
        coin.firstElementChild.nextElementSibling.textContent ==
        coins.coins[x].name
      ) {
