...
if(localStorage.getItem('dotTradeLogBuy')) {
    trade.tradeLogBuy = JSON.parse(localStorage.getItem('dotTradeLogBuy'));
}

if(localStorage.getItem('dotTradeLogSell')) {
    trade.tradeLogSell = JSON.parse(localStorage.getItem('dotTradeLogSell'));
}
...
const dotPriceDOM = document.querySelector('.coin-price');

setInterval(() => {
    trade.coins.map((coin) => {
        let coinsLastPrice = coin.price

        if(coin.symbol == "DOT-USDT") {
...
const currentDotDOM = document.querySelector('.current-coin');

trade.coins.map((coin) => {
    if(coin.symbol == "DOT-USDT") {
        currentDotDOM.innerHTML = `${coin.amount.toFixed(2)} DOT`;
        dotPriceDOM.innerHTML = `$ ${coin.price.toFixed(2)}`;
    }
})
...
buy = (e) => {
    ...
    if(money >= price) {
        ...
        trade.coins.map((coin) => {
            ...
            localStorage.setItem('dotTradeLogBuy', JSON.stringify(trade.tradeLogBuy));
        }
        });
    } else {
    ...
    trade.coins.map((coin) => {
        if (coin.symbol == "DOT-USDT") {
            if(parseInt(amount) <= coin.amount) {
                ...
                localStorage.setItem('dotTradeLogSell', JSON.stringify(trade.tradeLogSell));

                currentDotDOM.textContent = `${coin.amount.toFixed(2)} DOT`;
                moneyDOM.textContent = `${money.toFixed(2)} USDT`
            } else {
